...
class SimpleAdminTemplate(models.Model):
    ...
    def __str__(self):
        return 'SimpleAdminTemplate %s' % (self.display_name, )
    ...
    def config_schema(self):
        ...
        def is_userinput(template, field_key):
            return isinstance(template[field_key], dict) and template[field_key].get('simpleAdminMethod') == 'userInput'

        def is_injectable(template, field_key):
            return isinstance(template[field_key], dict) and template[field_key].get('simpleAdminMethod') == 'injectablePlugin'
        ...
        def create_schema(schema, template, root=False, hide_fields=None):
            ...
            for key, field in schema._declared_fields.items():
                ...
                if hide_fields is not None:
                    ...
                    if nested_hide_fields:
                        nested_schema, nested_added_fields = create_schema(field.nested, True, hide_fields=nested_hide_fields)
                    ...
                    nested_schema, nested_added_fields = create_schema(field.nested, template[key])
            ...
            if isinstance(template, dict):
                for key, template_field in template.items():
                    if is_injectable(template, key):
                        plugin_type = pluginhandler.get_plugin_type(template_field['plugin_type'])
                        traits = template_field.get('traits', [])
                        schema_keys[key] = RelatedPluginField(required=True, plugin_type=plugin_type, traits=traits)

            if root and self.update_method == self.UPDATE_METHOD_FULL and 'display_name' not in schema_keys:
                #schema_keys['name'] = fields.String(required=True, pattern=r'^[a-z0-9]+(?:[-_][a-z0-9]+)*$', ui_schema={'ui:title': 'Name'})
                schema_keys['display_name'] = fields.String(required=True, default='New unsaved plugin', ui_schema={'ui:title': 'Name'})
            ...
            existing_ui_order = ['display_name'] + existing_ui_schema.get('ui:order', [])
            def get_index(f):
                try:
            ...
            if root and self.update_method == self.UPDATE_METHOD_MODIFY_KEY:
                schema_keys[self.modify_key] = RelatedPluginField(required=True, plugin_name=plugin, ui_schema={'ui:title': 'Items'})
    ...
    def add_plugin(self, name, items=None):
        if self.update_method == self.UPDATE_METHOD_FULL:
            try:
                ...
                raise NameAlreadyInUseException('Name %s is already used' % (name, ))
...
class SimpleAdminPlugin(models.Model):
    ...
    auto_created_plugins = models.ManyToManyField(Plugin, related_name='auto_created_plugins', blank=True)
    name = models.CharField(max_length=200, blank=True, null=True, help_text='Slugified name, used for actual name (and be able to find the plugin again)')
    display_name = models.CharField(max_length=200, blank=True, null=True, help_text='Human readable name')
    ...
    def delete_plugin(self):
        for auto_created_plugin in self.auto_created_plugins.all():
            logger.debug('Deleting auto-created plugin %r from %r' % (auto_created_plugin, self.plugin, ))
        ...
        if self.template.update_method == self.template.UPDATE_METHOD_FULL:
            logger.debug('Removing plugin %s completely with plugin %s' % (self.plugin, self,))
            self.plugin.remove_plugin()
            self.plugin.delete()
        elif self.template.update_method == self.template.UPDATE_METHOD_MODIFY_KEY:
            logger.debug('Removing a key plugin from %s with plugin %s' % (self.plugin, self,))
            found_item = False
            for i, item in enumerate(self.plugin.config.get(self.template.modify_key, [])):
                if item['name'] == self.name or item.get('_simpleadminplugin_id') == self.pk:
                    found_item = True
                    break
    ...
    def update_plugin(self, form_data): # TODO: maybe make it choose a new name if already in use?
        """
        ...
        display_name = form_data.get('display_name') or filled_form_data.get('display_name') or self.display_name
        assert name
        ...
        if name != self.name:
            if self.template.update_method == self.template.UPDATE_METHOD_FULL:
                try:
                    ...
                    raise NameAlreadyInUseException('Unable to rename because name %s is already used' % (name, ))
                except Plugin.DoesNotExist:
                    pass
            elif self.template.update_method == self.template.UPDATE_METHOD_MODIFY_KEY:
                if key_name_in_use(self.plugin, name):
                    raise NameAlreadyInUseException('Unable to save because name %s is already used' % (name, ))

        should_reload = False

        if self.template.update_method == self.template.UPDATE_METHOD_FULL:
            logger.debug('Doing a full fill of form data to %s from %s' % (self.plugin, self,))
            self.plugin.config = filled_form_data
            self.plugin.name = name
            should_reload = True
        elif self.template.update_method == self.template.UPDATE_METHOD_MODIFY_KEY:
            logger.debug('Doing a partial fill of form data to %s from %s' % (self.plugin, self,))
            ...
            if self.plugin.config.get(self.template.modify_key):
                ...
                if not updated_item:
                    self.plugin.config[self.template.modify_key].append(filled_form_data)
            else:
    ...
    def fill_form_data(self, form_data):
        ...
        def create_plugin(template):
            if self.pk is not None:
                ...
                config = {k: v  % {'name': self.pk} for (k, v) in template.get('config', {}).items()}
            ...
            if isinstance(template, dict):
                ...
                if method:
                    if method == 'userInput':
                        ...
                        plugins = Plugin.objects.filter(plugin_type=template['plugin_type'], name=template['name'], enabled=True)
                        if plugins:
                            return plugins[0].pk
                        else:
                            logger.warning('Unable to find plugin plugin_type:%s name:%s' % (template['plugin_type'], template['name']))
                            return None
                    elif method == 'lookupAllPlugins':
                        return Plugin.objects.filter(plugin_type=template['plugin_type'], enabled=True).values_list('pk', flat=True)
                        ...
                        for plugin_template in template['pluginPriorities']:
                            if not pluginhandler.get_plugin(plugin_template['plugin_type'], plugin_template['plugin_name']):
                                continue
                    ...
                    for k, v in template.items():
                        if isinstance(v, list):
                            ...
                            source_value = None
                            if isinstance(source_form_data, dict) and source_form_data.get(k) is not None:
        ...
        if self.template.template == True:
            ...
            if isinstance(template, dict) and 'name' not in template and 'display_name' in template:
                template['name'] = {'simpleAdminMethod': 'slugify', 'source': 'display_name'}
    ...
    def get_display_name(self):
        ...
        return self.display_name or config.get('display_name') or self.name or 'No name found'

    def __str__(self):
        return 'SimpleAdminPlugin %s' % (self.name, )
