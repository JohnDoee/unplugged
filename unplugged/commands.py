...
class CommandViewMixin:
    def call_command(self, request, obj, additional_kwargs=None):
        ...
        if not serializer.is_valid():
            jsonapi_root = JSONAPIRoot.error_status(id_='deserialize_failed',
                                                    detail='Failed to deserialize your request')
        ...
        if not command:
            jsonapi_root = JSONAPIRoot.error_status(id_='unknown_command',
                                                    detail="%s is not a known command" % (command_name, ))
            jsonapi_root.meta.update(serializer.errors)
            return Response(jsonapi_root.serialize(request),
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if result.errors:
            jsonapi_root = JSONAPIRoot.error_status(id_='invalid_args',
                                                    detail="You provided invalid arguments to the function %s" % (command,))
        ...
        try:
            ...
            logger.exception('Failed to execute %s with args %r' % (command, result.data))
            jsonapi_root = JSONAPIRoot.error_status(id_='execution_failed',
...
class CommandBase:
    ...
    def get_jsonapi_commands(cls):
        ...
        serialized_commands = []
        for command_obj, command in zip(CommandSerializer(commands.values(), many=True).data, commands.values()):
...
class Command:
    def __init__(self, fn, name=None, display_name=None, description=None, schema=None, metadata=None, need_request=False):
        self.fn = fn
        if name:
            self.name = name
        else:
...
def command(name=None, display_name=None, description=None, schema=None, metadata=None, need_request=False):
    def decorator(fn):
        fn.__command__ = Command(fn, name, display_name, description, schema, metadata, need_request)
        @wraps(fn)
...
class CommandSerializer(serializers.Serializer):
    ...
    def get_schema(self, obj):
        ...
        return json_schema.dump(schema).data
