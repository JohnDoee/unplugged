...
from ..jsonapi import JSONAPIRoot, JSONAPIObject
from ..jsonschema import dump_ui_schema
from ..libs.marshmallow_jsonschema import JSONSchema
from ..models import SimpleAdminTemplate, SimpleAdminPlugin, NameAlreadyInUseException
...
class SimpleAdminTemplateSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ('display_name', 'description', 'template_id', 'plugin_type', 'plugin_name', 'schema', 'ui_schema', )
    ...
    def get_schema(self, obj):
        ...
        return json_schema.dump(obj.config_schema()).data
...
class SimpleAdminPluginSerializer(serializers.HyperlinkedModelSerializer):
    url = serializers.HyperlinkedIdentityField(view_name="simpleadmin_plugin-detail")
    ...
    class Meta:
        ...
        fields = ('name', 'template', 'template_id', 'plugin_id', 'plugin_type', 'plugin_name', 'url', 'config', 'display_name', )
...
class SimpleAdminPluginModelView(viewsets.ModelViewSet):
    ...
    def set_priorities(self, request):
        ...
        for plugin in plugins:
            def get_priority(p):
                ...
                return 100
            plugin.config[sap.template.modify_key] = sorted(plugin.config[sap.template.modify_key], key=get_priority)
            plugin.save()
    ...
    def perform_create(self, serializer):
        template = get_object_or_404(SimpleAdminTemplate, pk=serializer.initial_data['template_id'])
        ...
        try:
            sap = template.add_plugin(name, serializer.initial_data['config'].get(template.modify_key))
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer_class = super(SimpleAdminPluginModelView, self).get_serializer_class()
        return Response(serializer_class(instance=instance, context={'request': request, 'view': self}).data)
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer_class = super(SimpleAdminPluginModelView, self).get_serializer_class()
        return Response(serializer_class(instance=obj, context={'request': request, 'view': self}).data, status=status.HTTP_201_CREATED, headers=headers)
...
class ShowAdminUrlsView(APIView):
    ...
    def get(self, request):
        ...
        # for url in self.router.get_urls():
        for url in self.urls:
            ...
            if url.name.endswith('-list'):
                ...
                view_id = '%s_%s' % (resource_name, url.name.split('-', 1)[1].replace('-', '_'), )
