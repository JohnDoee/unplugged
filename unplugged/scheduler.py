...
class ScheduleManager:
    ...
    def plugin_loaded(self, sender, plugin, **kwargs):
        logger.debug('Creating schedules for %s' % (plugin, ))
    ...
    def plugin_unloaded(self, sender, plugin, **kwargs):
        logger.debug('Removing schedules for %s' % (plugin, ))
        for schedule in Schedule.objects.filter(plugin=plugin, enabled=True):
            self.unload_schedule(schedule)

    def unload_schedule(self, schedule):
        logger.debug('Unloading schedule %r' % (schedule, ))
    ...
    def trigger_schedule(self, pk):
        ...
        if not schedule.enabled:
            logger.warning('Trying to trigger disabled schedule %s' % (schedule.pk, ))
            return
        ...
        try:
            ...
            logger.exception('Failed to execute %s with args %r from schedule' % (command, kwargs))
